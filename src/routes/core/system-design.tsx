        ...
        <div className="flex flex-wrap gap-3">
          {TECH_STACK.map(
            ({ name, version, color }) => (
              <div
                key={name}
                className={`flex items-center gap-2 rounded-2xl border px-4 py-2 text-xs sm:text-sm font-medium transition-all hover:-translate-y-1 hover:shadow-lg ${color}`}
              >
                <span>{name}</span>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {(
            ...
            <button
              ...
              className={`group rounded-3xl border bg-gradient-to-br ${layer.gradient} p-5 sm:p-6 text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-xl backdrop-blur ${
                layer.border
              } ${
                activeLayer === key
                  ? 'scale-[1.02]'
          ...
          {[
            ...
            ({ title, detail, key }) => (
              <button
                ...
                className="rounded-2xl border border-border bg-card/60 p-5 text-left transition-all hover:-translate-y-1 hover:border-primary/20 hover:shadow-lg"
              >
